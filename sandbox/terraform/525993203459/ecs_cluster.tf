...
resource "aws_ecs_service" "web_app_service" {
  ...
  depends_on      = [aws_iam_role_policy.ecs_policy, aws_ecs_task_definition.web_app_task]
  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "${var.ACCOUNT_ID}.dkr.ecr.${var.aws_region}.amazonaws.com/${aws_ecr_repository.web_app_repo.name}:latest"
      cpu       = 10
      memory    = 512
      essential = true
      portMappings = [
...
resource "aws_launch_configuration" "ecs_launch_config" {
  ...
  user_data            = "#!/bin/bash\necho ECS_CLUSTER=${aws_ecs_cluster.web_app_cluster.name} >> /etc/ecs/ecs.config"
